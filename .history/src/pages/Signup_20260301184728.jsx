...
const Signup = () => {
  ...
  const PasswordInput = ({ label, name, value, onChange, show, onToggle, placeholder, error }) => (
    <div className="flex flex-col gap-1">
      <label htmlFor={name} className="text-xs text-gray-700 font-medium tracking-wide uppercase">
        {label}
      </label>
      ...
      {/* Match feedback for confirm password */}
      {name === 'confirmPassword' && value && (
        <p className={`text-xs mt-1.5 ${form.password === value ? 'text-green-600' : 'text-red-600'}`}>
          {form.password === value ? '✓ Passwords match' : '✗ Passwords do not match'}
        </p>
      )}
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col lg:flex-row">
      ...
      {/* Right: Brand panel - visible on lg+ */}
      <div
