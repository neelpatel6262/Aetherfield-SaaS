...
import { Eye, EyeOff, Check, X, CircleCheck } from 'lucide-react';
...
const SignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ password stripped before storing
      ...
      <div className="min-h-screen bg-white flex items-center justify-center px-5">
        <div className="w-full max-w-md text-center flex flex-col items-center gap-6">
          ...
          <div className="border-t border-gray-100 pt-6 w-full flex flex-col gap-3 text-sm text-gray-500">
            <button onClick={() => setStep(1)} className="hover:text-black transition underline">
              Wrong email? Go back
            </button>
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col lg:flex-row">
      ...
      <div className="w-full lg:w-1/2 flex flex-col px-6 md:px-12 lg:px-16 py-10 lg:py-16 overflow-y-auto">
        <div className="mb-10">
          <h1 className="text-4xl md:text-5xl font-extrabold text-black leading-tight font-[heading]">
            Create your account
          </h1>
          <p className="mt-3 text-base text-gray-600 font-[subheading]">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5" noValidate>
          ...
          <div className="flex items-start gap-3 mt-2">
            ...
            <label className="text-sm text-gray-600 cursor-pointer select-none font-[subheading]">
              I agree to Aetherfield's{' '}
              <Link to="/terms"   className="text-black underline hover:opacity-80">Terms of Service</Link>{' '}
              and{' '}
              <Link to="/privacy" className="text-black underline hover:opacity-80">Privacy Policy</Link>
            </label>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className={`mt-6 w-full bg-black text-white text-base font-medium py-4 transition font-[btn]
              ${loading ? 'opacity-60 cursor-not-allowed' : 'hover:bg-black/90'}`}
          >
        ...
        <div className="flex gap-12 text-black/60">
          {[
            ...
            <div key={stat.label} className="space-y-1">
              <p className="text-5xl font-extrabold text-black font-[heading]">{stat.number}</p>
              <p className="text-sm text-black/60 font-[subheading]">{stat.label}</p>
            </div>
          ))}
        </div>
      </div>
