...
function Navbar() {
  ...
  // Stagger animation for desktop/mobile menu items
  const container = {
    hidden: { opacity: 0 },
    show: {
      opacity: 1,
      transition: { staggerChildren: 0.08 }, // gentle delay between items
    },
  }
  ...
  return (
    <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-100">
      <nav className="w-full px-10 py-4 flex items-center justify-between">
        ...
        {/* Mobile toggle button – subtle scale on click */}
        <motion.button
          className="md:hidden flex flex-col justify-center items-center gap-[5px] w-8 h-8"
          onClick={() => setOpen(!open)}
          aria-label="Toggle menu"
          whileTap={{ scale: 0.95 }} // tiny press feedback
        >
      ...
      {/* Mobile dropdown – slide down + fade */}
      <motion.div
